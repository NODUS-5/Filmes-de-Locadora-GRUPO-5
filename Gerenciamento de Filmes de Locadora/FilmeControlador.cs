        ...
        public static void AdicionarFilme()
        {
            while (true) {
                try
                {
                    ...
                    int anoLancamento;
                    while (true) { //loop interno para tratar erro separadamente
                        Console.Write("\nDigite o ano de lançamento do filme: "); anoLancamento = int.Parse(Console.ReadLine());

                        if (anoLancamento > Utils.ObterAnoAtual() || anoLancamento < Utils.anoPrimeiroFilme) //utilizando dados da classe utils para obter as validações
                        {
                    ...
                    int duracaoFilme;
                    while (true)
                    { //loop interno para tratar erro separadamente
                        Console.Write("\n Digite a duração do filme em minutos: "); duracaoFilme = int.Parse(Console.ReadLine());
                        if (duracaoFilme <= 0) { Console.WriteLine("Por favor, digite um valor maior que 0."); } else { break; }//verificação se o número é menor ou igual a 0
                    }
                    if (tituloFilme == "" || diretorFilme == "" || generoFilme == "" || classIndicativa == "") { Console.WriteLine("Por favor, não deixe o campos em branco."); } //validação de conteudo em branco
                    else {
            ...
            while (true)
            {
                try 
                {
                    ...
                    if (filmeEditar <= 0) 
                    {
                        ...
                        if (FilmeDAO.ExisteId(filmeEditar)) //utilizando o método de verificação de id
                        {
                            ...
                            if (opcaoCampo < 1 || opcaoCampo > 6)//validação se o número se encaixa no campos proposto
                            {
                            ...
                            else
                            {
                                switch (opcaoCampo)
                                {
                                    case 1:
                                        ...
                                        if (Utils.ConfirmarOperacao()) { FilmeDAO.AtualizarCampoFilme(filmeEditar, "titulo", novoTitulo); } //utilizando a classe utils para confirmação do usuario
                                        break;
                                    case 2:
                                        Console.Write("Digite o novo Diretor: ");
                                        string novoDiretor = Console.ReadLine();
                                        if (Utils.ConfirmarOperacao()) { FilmeDAO.AtualizarCampoFilme(filmeEditar, "diretor", novoDiretor); }//utilizando a classe utils para confirmação do usuario
                                        break;
                                    case 3:
                                        Console.Write("Digite o novo Gênero: ");
                                        string novoGenero = Console.ReadLine();
                                        if (Utils.ConfirmarOperacao()) { FilmeDAO.AtualizarCampoFilme(filmeEditar, "genero", novoGenero); }//utilizando a classe utils para confirmação do usuario
                                        break;
                                    case 4:
                                        Console.Write("Digite o novo Ano de Lançamento: ");
                                        string novoAno = Console.ReadLine();
                                        if (Utils.ConfirmarOperacao()) { FilmeDAO.AtualizarCampoFilme(filmeEditar, "ano_lancamento", novoAno, true); }//utilizando a classe utils para confirmação do usuario
                                        break;
                                    case 5:
                                        Console.Write("Digite a nova Classificação Indicativa: ");
                                        string novaClassificacao = Console.ReadLine();
                                        if (Utils.ConfirmarOperacao()) { FilmeDAO.AtualizarCampoFilme(filmeEditar, "classificacao_indicativa", novaClassificacao); }//utilizando a classe utils para confirmação do usuario
                                        break;
                                    case 6:
                                        Console.Write("Digite a nova Duração do filme (em minutos): ");
                                        string novaDuracao = Console.ReadLine();
                                        if (Utils.ConfirmarOperacao()) { FilmeDAO.AtualizarCampoFilme(filmeEditar, "duracao_minutos", novaDuracao, true); }//utilizando a classe utils para confirmação do usuario
                                        break;
                                    default:
        ...
        public static void RemoverFilme()
        {
            while (true)
            {
                try
                {
                    ...
                    if (id <= 0) // validação de numeros negativos e zero
                    {
                        Console.WriteLine("Por favor digite um id válido.");
                    }
                    else
                    {
                        if (FilmeDAO.ExisteId(id)) //validando com o método de verificação de existencia do id
                        {
                            if (Utils.ConfirmarOperacao())//Validação de confirmação
                            {
        ...
        public static void BuscarFilmesPor()
        {
            while (true)
            {
                try {
                    ...
                    if (opcaoFiltro < 1 || opcaoFiltro > 7) // validação se o númmero se encaixa no campo proposto
                    {
                        Console.WriteLine("Digite um número entre 1 e 7.");
                    }
                    else
